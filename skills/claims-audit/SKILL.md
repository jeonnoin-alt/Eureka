...
|---|---|---|---|---|---|
| Fig 1 | scripts/plot_fig1_connectome.py | YES | YES | NO | OK |
...
|---|---|---|---|---|
| Baseline NDM | YES | YES | YES (r = 0.72) | OK |
| Baseline ESM | YES | YES | YES (r = 0.79) | OK |
| Proposed AND model | YES | YES | YES (r = 0.86) | OK |
| Ablation: no gene expression | YES | NO | NO (r = 0.61) | CRITICAL: unreported negative result |
| Ablation: no structural connectivity | YES | YES | NO (r = 0.58) | OK (reported in Supp Table 2) |
| Sensitivity: alternate parcellation | YES | NO | NO (p = 0.18) | CRITICAL: unreported null result |
| Sensitivity: alternate seed region | YES | NO | YES (r = 0.84) | WARNING: unreported significant variant |
```
...
Read the manuscript section by section. Create a raw list of every quantitative claim before looking up any source files. Work from the text alone — do not let knowledge of the results directory bias which claims you record.

### Step 2: Trace each claim to a source file

For each claim in your list, find the source file. Start in `results/`. If not found there, check: `notebooks/` output cells, `logs/`, inline in scripts (not acceptable — this is a flag). Record the source file path and the exact location within the file.

### Step 3: Verify each number matches

Compare the manuscript value to the source value. Record match status. Any discrepancy is CRITICAL.
